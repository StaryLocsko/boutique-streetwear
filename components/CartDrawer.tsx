...
export default function CartDrawer() {
  ...
  return (
    <>
      ...
      />
      <aside
        className={`fixed right-0 top-0 z-50 flex h-full w-full max-w-md transform flex-col border-l border-white/[0.08] bg-[#0c0d10]/95 backdrop-blur-xl transition-transform duration-300 ${
          isOpen ? "translate-x-0" : "translate-x-full"
        }`}
        aria-hidden={!isOpen}
      >
        <div className="flex items-center justify-between border-b border-white/[0.08] px-5 py-4">
          <h2 className="text-sm font-semibold uppercase tracking-[0.2em] text-white">Panier</h2>
          <button
            type="button"
            onClick={closeCart}
            ...
            <ul className="flex flex-col gap-4">
              {items.map((item) => (
                <li key={`${item.product.id}-${item.size}`} className="flex gap-3">
                  <div className="relative h-20 w-16 shrink-0 overflow-hidden rounded-lg border border-white/[0.08] bg-white/[0.02]">
                    <Image
        ...
        <div className="flex flex-col gap-3 border-t border-white/[0.08] px-5 py-4">
          {items.length > 0 && (
            <div className="flex flex-col gap-1.5">
              ...
              <div className="mt-1 flex justify-between border-t border-white/[0.08] pt-2 text-sm text-zinc-100">
          ...
          {whatsappLink ? (
            <a
              ...
              className="flex w-full items-center justify-center gap-2 rounded-xl border border-white/10 bg-[#25D366] py-3.5 text-center text-xs font-semibold uppercase tracking-[0.15em] text-zinc-950 transition-colors hover:bg-[#1ebe5b]"
            >
            ...
            <span className="w-full cursor-not-allowed rounded-xl border border-white/[0.08] bg-white/[0.03] py-3.5 text-center text-xs font-semibold uppercase tracking-[0.15em] text-zinc-500">
              Commander sur WhatsApp
            </span>
          )}
        </div>
