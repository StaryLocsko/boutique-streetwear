...
export default function FAQ() {
  ...
  return (
    <section id="faq" className="mt-16 border-t border-zinc-200/80 pt-10">
      <h2 className="mb-6 text-xl font-semibold text-zinc-950">Questions fréquentes</h2>
        ...
        {FAQ_ITEMS.map((item, index) => {
          ...
          return (
            <div key={item.question}>
              ...
              {isOpen && <p className="pb-4 text-sm leading-relaxed text-zinc-600">{item.answer}</p>}
            </div>
          );
        })}
      </div>
